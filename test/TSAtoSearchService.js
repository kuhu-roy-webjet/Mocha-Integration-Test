...
describe('Interaction between Insurance Search Service and TSA', function(){
 
it('should return correct response',function(){
return chai.request('https://services.dev.webjet.com.au/')
        .post('api/insurance/searchservice')
        .set('Content-Type','application/json')
        .set('X-ApiKey','BAAFD8EABA29FFBE65D185B5F66A3')
        ...
        .send({travelClass:'Economy'})
        .send({tripCost:394.38})
        .send({numberOfAdults:2})
        .send({numberOfChildren:1})
        .send({numberOfInfants:1})
        .send({numberOfDependants:2})
        .send({provider:"CoverMore"})
        .send({isFixedDuration:"true"})
